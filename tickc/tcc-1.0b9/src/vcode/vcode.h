...
typedef struct { unsigned l; unsigned h; } v_code;
#else
...
#if 0
/* True if automatically filling delay slots with nops */
extern unsigned int v_autonop;
#endif
...
v_reg_type v_rccalli(struct v_cstate *c, v_reg_type r);
...
v_reg_type v_rccallf(struct v_cstate *c, v_reg_type r);
...
/* v_pickreg: mark register reg of type type as allocated */
...
#ifndef __no_bpo__
/*
 * v_bpo_{on,off}: turn binary peephole optimizations on/off
 */

extern void v_bpo_on  (void);
extern void v_bpo_off (void);
#endif
...
/* Reset state after consuming closures: needed for benchmarking */
extern void v_reset_closure_state (void);
