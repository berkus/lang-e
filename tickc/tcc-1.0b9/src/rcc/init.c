...
/* genasgn - append tree for assignment of e to evolving structure expression
   in *sp */
static int genasgn(e, sp) Tree e; struct structexp *sp; {
     Tree p;

     sp->off = roundup(sp->off, e->type->size);
     p = simplify(ADD+P, ptr(e->type), lvalue(idtree(sp->var)), 
		  consttree(sp->off, inttype));
     if (isarray(e->type)) {
...
static void genchar(p, sp) Symbol p; struct structexp *sp; {
     ...
     if (n > 0) {
	  static Value v;
	  Type ty = array(chartype, p->type->size - (s - (char *)p->u.c.v.p), 
			  0);
	  Tree e;
...
static int genconst(e, def) Tree e; int def; {
     for (;;)
	  ...
	  case CVP:
	       if (isarith(e->type))
		    error("cast from `%t' to `%t' is illegal in "
	  ...
	  case CVD: case CVF:
	       ...
	       continue;
	  case ADD:
	       if (cbe.emit && cbe.flush && optype(e->op) == P)
		    return e->type->size;
	  default:
	       error("initializer must be constant\n");
	       if (def)
     ...
     do {
	  if (current) {
	       ...
	       if ((aty = assign(ty, e)) != NULL)
		    ...
		    error("invalid initialization type; found `%t' "
			  "expected `%s'\n", e->type, ty);
	       ++n;
	  } else {
...
/* initfields - initialize <= an unsigned's worth of bit fields in fields
   p to q */
static int initfields(p, q) Field p, q; {
     ...
     if (current) {
	  ...
	  do {
	       ...
	       e = (*optree['|'])(BOR, e,
				  (*optree[LSHIFT])
				  (LSH, x, consttree(fieldright(p), inttype)));
	       if (p->link == q)
		    break;
	       p = p->link;
	  } while (t == ',' && (t = gettok()) != 0);
	  if (q && (n = q->offset - p->offset) < unsignedtype->size)
	       if (IR->little_endian)
		    ...
		    for (i = n - 1; i >= 0; i--) {
			 Tree x = (*optree[RSHIFT])
			      (RSH, e, 
			       consttree(8*(unsignedtype->size - n + i), 
					 inttype));
...
Type initializer(ty, lev) Type ty; int lev; {
     /* Effects:  
	in `C-C mode: 	prints initializer iff cbe.flush == 1 */
     int n = 0;
     Tree e;
     Type aty = NULL;
