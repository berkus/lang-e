...
static char rcsid[] = "$Id: lburg.c,v 1.1.1.1 1997/10/30 16:17:14 maxp Exp $";
static char *prefix = "";
static int Tflag = 0;
...
Rule rule(char *id, Tree pattern, 
...
static void emitstring(Rule rules) {
#define isinst(x) ((x)&0x80000000)
	Rule r;

	if (ts == BINARY)
	     print("static unsigned int %Ptemplates[] = {\n");
	else
	...
	if (ts == BINARY)
	     for (r = rules; r; r = r->link)
		  print("/* %d */%1%d,\n", r->ern, isinst(r->bintemp));
	else 
	     for (r = rules; r; r = r->link) {
		  int len = strlen(r->asctemp);
		  print("/* %d */%1%d,%1/* %s */\n", r->ern,
			(len >= 2 && r->asctemp[len-2] == '\\' 
			 && r->asctemp[len-1] == 'n'),
			r->asctemp);
	     }
	print("};\n");
