...
/* need to be much more clever.  BLech. */
void set64u(unsigned Rdst, unsigned long val) {
	unsigned lo;
	int rd;
	int hi;
...
#if 0
(ra, disp) BRANCH(v_ip, @bin, ra, disp); {
        br   bsr
        0x30 0x34
}
#endif

/* 21-bit signed offset is added to the subsequent PC. */
void br(int ra, void *addr) {
	unsigned offset;
	long disp;

	/* Reduce displacement to 21 bits. */
	disp = (unsigned *)addr - (unsigned *)v_ip - 1;
	if(disp >= 0) 
		offset = 0;
	else {
		disp = -disp;
		offset = 1 << 21; /* sign bit */
	}
	offset |= disp;

	/* Encode */
	if((unsigned long)disp >> 20) {
		/* didn't fit. */
		v_setp(v_at, addr);
		v_jp(v_at);
	} else {
		BRANCH(v_ip, 0x30, ra, offset);
	}
}
