...
static void bmul(struct v_reg rd, struct v_reg rs, struct v_reg t1, unsigned x) ;
 ...
 */
static void mgen(struct v_reg rd, struct v_reg rs, struct v_reg t, unsigned x) {
	char *p = mlookup[x];
	struct v_reg rr, t2, lastr;
	int equal;
...
#if 0
static void emit_multiply(struct v_reg rd, struct v_reg rs, unsigned x) {
	int cost, seq[10];
